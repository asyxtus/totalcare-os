...
export default function TriageForm({ visitId, clinicId }: TriageFormProps) {
  ...
  return (
    <form action={handleSubmit}>
      {queued && (
        <p role="status" style={{ fontSize: '13px', color: 'var(--color-warning-text)', background: 'var(--color-warning-bg)', padding: '8px 10px', borderRadius: 'var(--radius-sm)', marginBottom: '1rem' }}>
          {t.queued}
        </p>
      )}
