...
import { MapPin, Search, ArrowRight, AlertCircle, WifiOff } from 'lucide-react'
...
// Journey data includes financial state, so keep a short TTL and isolate the
// local index by clinic. This is deliberately much shorter than the generic
// cache TTL used for non-sensitive reference data.
const JOURNEY_CACHE_TTL_MS = 15 * 60 * 1000
const cacheKey = (clinicId: string) => `reception:patient-journey:${clinicId}`

function money (value: number, lang: 'fr'|'en') {
 return `${new Intl.NumberFormat(lang==='fr'?'fr-FR':'en-US').format(value)} FCFA`
}

function FinancialAlert({r,lang,cached}:{r:PatientJourneyResult;lang:'fr'|'en';cached:boolean}) {
 ...
 return <div style={{marginTop:10,padding:11,border:'1px solid var(--color-warning-border)',background:'var(--color-warning-bg)',borderRadius:'var(--radius-sm)'}}>
  ...
  <div style={{fontSize:10,color:'var(--color-text-secondary)',marginTop:3}}>{r.financial.charge_count} {lang==='fr'?'facture(s) impayée(s)':'outstanding charge(s)'}{cached ? (lang==='fr'?' · Données en cache':' · Cached data') : ''}</div>
  {!cached && <Link href={`/billing/patients/${r.patient.id}`} style={{display:'inline-flex',alignItems:'center',gap:5,marginTop:8,padding:'6px 9px',border:'1px solid var(--color-border)',borderRadius:'var(--radius-sm)',background:'var(--color-surface)',color:'var(--color-text)',textDecoration:'none',fontSize:11,fontWeight:600}}>{lang==='fr'?'Voir le compte / encaisser':'View account / collect'} <ArrowRight size={12}/></Link>}
 </div>
}

function Card({r,lang,cached}:{r:PatientJourneyResult;lang:'fr'|'en';cached:boolean}) {
 const j=r.journey
 ...
 return <div style={{border:'1px solid var(--color-border)',borderRadius:'var(--radius-md)',padding:14,marginTop:10,background:'var(--color-surface)'}}>
  ...
  <FinancialAlert r={r} lang={lang} cached={cached}/>
...
export default function EncounterJourneyLookup({lang,clinicId}:{lang:'fr'|'en';clinicId:string}) {
 const [q,setQ]=useState(''); const [rows,setRows]=useState<PatientJourneyResult[]>([]); const [busy,setBusy]=useState(false); const [searched,setSearched]=useState(false); const [error,setError]=useState<string|null>(null); const [cached,setCached]=useState(false)

 async function run(){
  const query=q.trim()
  if(query.length<2) return
  setBusy(true); setSearched(true); setError(null); setCached(false)
  try {
   ...
   const local=await getOfflineCache<PatientJourneyResult[]>(cacheKey(clinicId))
   const needle=query.toLocaleLowerCase()
   const matches=(local??[]).filter(r => `${r.patient.full_name} ${r.patient.patient_code}`.toLocaleLowerCase().includes(needle)).slice(0,10)
   if(matches.length){ setRows(matches); setCached(true); setError(null) }
   else { setRows([]); setError(lang==='fr'?'Réseau indisponible et aucun patient correspondant n’est en cache.':'Network unavailable and no matching patient is cached.') }
  } finally { setBusy(false) }
 }

 return <section style={{marginBottom:18}}>
  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><MapPin size={16}/><b style={{fontSize:14}}>{lang==='fr'?'Parcours patient':'Patient journey'}</b><span style={{fontSize:10,color:'var(--color-text-secondary)'}}>{lang==='fr'?'Localiser le patient dans son parcours':'Find the patient in their journey'}</span></div>
  <div style={{display:'flex',gap:8}}><div style={{position:'relative',flex:1}}><Search size={14} style={{position:'absolute',left:9,top:10,color:'var(--color-text-secondary)'}}/><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&run()} placeholder={lang==='fr'?'Nom ou code patient…':'Patient name or code…'} style={{width:'100%',boxSizing:'border-box',padding:'8px 9px 8px 30px',border:'1px solid var(--color-border)',borderRadius:'var(--radius-sm)',background:'var(--color-surface)',color:'var(--color-text)',fontSize:12}}/></div><button type='button' disabled={busy||q.trim().length<2} onClick={run} style={{border:0,borderRadius:'var(--radius-sm)',padding:'0 13px',background:'var(--color-accent)',color:'var(--color-accent-text-on)',fontSize:11,fontWeight:600}}>{busy?'…':lang==='fr'?'Rechercher':'Search'}</button></div>
  {cached&&<div role='status' style={{display:'inline-flex',alignItems:'center',gap:6,marginTop:8,fontSize:10,fontWeight:600,color:'var(--color-warning-text)'}}><WifiOff size={12}/>{lang==='fr'?'Hors ligne · résultats locaux · données potentiellement obsolètes':'Offline · local results · data may be stale'}</div>}
  {error&&<p style={{fontSize:12,color:'var(--color-critical-text)'}}>{error}</p>}
  {searched&&!busy&&!error&&!rows.length&&<p style={{fontSize:12,color:'var(--color-text-secondary)'}}>{lang==='fr'?'Aucun parcours actif ou solde impayé trouvé.':'No active encounter or outstanding balance found.'}</p>}
  {rows.map(r=><Card key={`${r.patient.id}-${r.journey?.visit_id??'balance'}`} r={r} lang={lang} cached={cached}/>)}
 </section>
}
