...
// components/AddChargeForm.tsx

import { useState } from 'react'
import { addManualChargeAction } from '@/lib/actions/billing'
...
export default function AddChargeForm({
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} style={{
        ...
        {lang === 'fr' ? '+ Ajouter une charge' : '+ Add charge'}
      </button>
    )
  }
  ...
  return (
    <form action={handleSubmit} style={{
      ...
      <input name="description" placeholder={lang==="fr"?"Description (ex. Certificat médical)":"Description (e.g. Medical certificate)"} required style={{ ...inputStyle, flex: 1, minWidth: '200px' }} />
