    ...
    <form action={handleSubmit} style={{
      ...
      {rows.map((row, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: '6px', marginBottom: '6px' }}>
          ...
          <input name="item_batch_number" value={row.batchNumber} onChange={(e) => updateRow(i, { batchNumber: e.target.value })} placeholder={lang==="fr"?"N° de lot":"Batch no."} style={inputStyle} />
          <input name="item_expiry_date" type="date" value={row.expiryDate} onChange={(e) => updateRow(i, { expiryDate: e.target.value })} style={inputStyle} />
          ...
          <input name="item_unit_cost" type="number" step="any" value={row.unitCost} onChange={(e) => updateRow(i, { unitCost: e.target.value })} placeholder={lang === 'fr' ? 'Coût unitaire' : 'Unit cost'} style={inputStyle} />
        </div>
      ))}
      ...
      <button type="button" onClick={() => setRows((r) => [...r, emptyRow()])} style={{
        ...
        padding: '5px 10px', borderRadius: 'var(--radius-sm)', fontSize: '11px', cursor: 'pointer', marginBottom: '12px',
      }}>
      ...
      {error && <p style={{ fontSize: '13px', color: 'var(--color-critical-text)', background: 'var(--color-critical-bg)', padding: '8px 12px', borderRadius: 'var(--radius-sm)', marginBottom: '10px' }}>{error}</p>}
      {success && <p style={{ fontSize: '13px', color: 'var(--color-success-text)', marginBottom: '10px' }}>✓ Réception enregistrée — le stock est à jour.</p>}

      <button type="submit" disabled={submitting} style={{
        fontSize: '13px', padding: '9px 16px', borderRadius: 'var(--radius-sm)', border: 'none',
        background: 'var(--color-accent)', color: 'var(--color-accent-text-on)', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.6 : 1,
      }}>
