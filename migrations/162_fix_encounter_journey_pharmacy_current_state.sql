-- ============================================================================
-- 162 — FIX PHARMACY JOURNEY CURRENT STATE
--
-- waiting_pharmacy is an encounter location, but the pharmacy task may already
-- be completed. Do not display the generic "Pharmacy action pending" message
-- when every prescribed quantity has been dispensed.
--
-- Also make the appointment blocker depend on the actual unresolved next task,
-- not merely on visits.status = waiting_pharmacy.
-- ============================================================================
...
begin
  ...
  -- Keep the displayed CURRENT action consistent with the actual pharmacy task.
  -- This is critical when visits.status remains waiting_pharmacy after all
  -- prescription quantities have already been dispensed.
  if v_stage = 'waiting_pharmacy' then
    case v_pharmacy_state
      when 'completed' then
        v_action_fr := 'Pharmacie terminée';
        v_action_en := 'Pharmacy completed';
      when 'awaiting_review' then
        v_action_fr := 'Validation de l’ordonnance requise';
        v_action_en := 'Prescription review required';
      when 'awaiting_payment' then
        v_action_fr := 'Paiement requis avant délivrance';
        v_action_en := 'Payment required before dispensing';
      when 'ready_to_dispense' then
        v_action_fr := 'Prêt à être dispensé';
        v_action_en := 'Ready to dispense';
      else
        null;
    end case;
  end if;

  -- Laboratory task state. Use text casts so this resolver does not assume
  -- enum labels that may differ between deployments.
  select count(*),
  ...
  -- Determine next operational destination from the actual outstanding task.
  if v_stage = 'in_consultation' then
    if v_lab_count > 0 and v_lab_state <> 'completed' then
    ...
    elsif v_prescription_count > 0 and v_pharmacy_state <> 'completed' then
      v_next_stage := 'waiting_pharmacy';
      v_next_stage_label_fr := 'Pharmacie'; v_next_stage_label_en := 'Pharmacy';
      v_next_action_fr := case v_pharmacy_state when 'awaiting_review' then 'Validation de l’ordonnance requise' when 'awaiting_payment' then 'Paiement requis avant délivrance' when 'ready_to_dispense' then 'Prêt à être dispensé' else 'Ordonnance à prendre en charge' end;
      v_next_action_en := case v_pharmacy_state when 'awaiting_review' then 'Prescription review required' when 'awaiting_payment' then 'Payment required before dispensing' when 'ready_to_dispense' then 'Ready to dispense' else 'Prescription to process' end;
    ...
    if v_lab_state = 'completed' then
      if v_prescription_count > 0 and v_pharmacy_state <> 'completed' then
    ...
    if v_pharmacy_state = 'completed' then
      ...
      case v_pharmacy_state
        ...
        else v_next_action_fr := 'Pharmacie terminée'; v_next_action_en := 'Pharmacy completed';
      end case;
    end if;
  ...
  -- Appointment guard: only unresolved workflow work should block a new
  -- appointment. A stale waiting_pharmacy status with completed dispensing
  -- must not be reported as an active pharmacy task.
  if not v_is_terminal and v_next_stage is not null and v_next_stage <> 'discharged' then
    v_blocked_reason_fr := 'Le patient a encore un parcours actif : ' || coalesce(v_next_stage_label_fr, v_stage_label_fr) || '.';
    v_blocked_reason_en := 'The patient still has an active encounter: ' || coalesce(v_next_stage_label_en, v_stage_label_en) || '.';
  end if;
  ...
  return jsonb_build_object(
    ...
    'appointment', jsonb_build_object(
      'blocked', (v_blocked_reason_fr is not null),
