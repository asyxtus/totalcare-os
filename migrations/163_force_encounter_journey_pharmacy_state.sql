-- Fix the production resolver when migrations 162 or later were not applied.
-- This migration intentionally replaces the canonical resolver with the current
-- pharmacy-aware implementation. It derives current/next actions from actual
-- prescription quantities and financial state rather than trusting a stale
-- waiting_pharmacy visit status.
...
begin
  ...
  -- A stale waiting_pharmacy status must never override actual pharmacy work.
  ...
  -- Laboratory task state. Text casts avoid assuming enum labels.
  select count(*),
  ...
  -- Determine next operational destination from actual unfinished work.
  if v_stage = 'in_consultation' then
    ...
    if v_pharmacy_state = 'completed' then
      ...
      case v_pharmacy_state
        ...
        else v_next_action_fr := 'Ordonnance à prendre en charge'; v_next_action_en := 'Prescription to process';
      end case;
    end if;
