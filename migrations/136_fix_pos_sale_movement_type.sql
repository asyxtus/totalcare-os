-- ============================================================================
-- FIX: POS SALES MUST RECORD SALE STOCK MOVEMENTS
--
-- Migration 135 tightened POS validation but called dispense_fefo() without
-- its movement-type argument. dispense_fefo() therefore used its default
-- movement type: 'dispense'.
--
-- POS transactions must create stock_movements with movement_type = 'sale'.
-- This migration preserves the existing validation/atomicity from 135 and
-- explicitly passes 'sale'::stock_movement_type to dispense_fefo().
-- ============================================================================
...
declare
  ...
  v_on_hand integer;
begin
  -- The entire function runs in one PostgreSQL transaction. If any
  -- validation, stock movement, or line-item insert fails, the whole sale
  ...
  -- ========================================================================
  -- PASS 1: validate the complete cart before creating any sale rows.
  -- ========================================================================
  for v_cart_item in
  ...
  loop
    ...
    if not v_product.is_active then
      raise exception 'Product % is inactive and cannot be sold',
        v_cart_item->>'product_id';
    end if;

    if v_product.sale_price_xaf is null
       or v_product.sale_price_xaf <= 0 then
      raise exception 'Product % has no valid sale price',
        v_cart_item->>'product_id';
    end if;

    if v_product.is_controlled then
      raise exception
        'Controlled substances cannot be sold via POS — this must go through a prescription with a witness';
    end if;

    -- Only active, non-expired batches count as POS-usable stock.
  ...
  -- ========================================================================
  -- PASS 2: create the sale and consume stock atomically.
  -- ========================================================================
  ...
  loop
    ...
    -- CRITICAL: explicitly record POS stock consumption as SALE, not
    -- DISPENSE. The final argument is required because dispense_fefo()
    -- defaults to 'dispense' when the movement type is omitted.
