...
declare
  ...
  v_role text;
begin
  if p_invoice_id is null then raise exception 'Laboratory invoice is required'; end if;
  ...
  if p_total_amount_xaf is null or p_total_amount_xaf <= 0 then raise exception 'Invalid laboratory payment amount'; end if;

  select i.clinic_id, i.patient_id, i.visit_id
    into v_invoice_clinic_id, v_invoice_patient_id, v_invoice_visit_id
  from invoices i
  where i.id = p_invoice_id and i.status in ('unpaid','partial')
  for update;
  if v_invoice_clinic_id is null then raise exception 'Laboratory invoice not found or is already settled'; end if;

  select coalesce(active_role, role)::text into v_role
  from staff where id = p_received_by and clinic_id = v_invoice_clinic_id and is_active = true;
  ...
  -- Lock and validate every selected item. Duplicate IDs are normalized by the
  -- count below, so the same test cannot be paid twice in one request.
  select count(distinct x.item_id) into v_item_count
  from unnest(p_lab_order_item_ids) as x(item_id);

  foreach v_item_id in array p_lab_order_item_ids loop
    select loi.status::text, loi.billing_status::text, loi.service_charge_id, lo.visit_id
      into v_item_status, v_billing_status, v_service_charge_id, v_item_visit_id
    from lab_order_items loi
    join lab_orders lo on lo.id = loi.lab_order_id
    where loi.id = v_item_id and loi.clinic_id = v_invoice_clinic_id
    for update;

    if v_item_status is null then raise exception 'Laboratory investigation % was not found', v_item_id; end if;
    if v_item_visit_id <> v_invoice_visit_id then raise exception 'Selected laboratory investigations do not belong to this invoice/encounter'; end if;
    if v_item_status <> 'pending' then raise exception 'Laboratory investigation % is no longer pending', v_item_id; end if;
    ...
    if v_service_charge_id is null then raise exception 'Laboratory investigation % has no financial charge', v_item_id; end if;
    if not exists (select 1 from invoice_items ii where ii.invoice_id = p_invoice_id and ii.service_charge_id = v_service_charge_id) then
      raise exception 'Laboratory investigation % is not part of the selected invoice', v_item_id;
    end if;
    if not exists (
      select 1 from service_charges sc
      where sc.id = v_service_charge_id and sc.patient_id = v_invoice_patient_id and sc.visit_id = v_invoice_visit_id and sc.status <> 'void'
    ) then
      raise exception 'Financial charge for laboratory investigation % is missing or void', v_item_id;
    end if;
  end loop;

  -- The invoice created by prepare_selected_lab_payment must contain exactly
  -- these selected lab charges. This prevents create_payment() from allocating
  -- the money to an unrelated consultation, pharmacy or other charge.
  select count(*) into v_invoice_item_count from invoice_items where invoice_id = p_invoice_id;
  ...
  if exists (
    ...
    where ii.invoice_id = p_invoice_id
      and not exists (
        ...
        where loi.id = any(p_lab_order_item_ids) and loi.service_charge_id = ii.service_charge_id
      )
  ) then
    raise exception 'The laboratory invoice does not exactly match the selected investigations';
  end if;

  select coalesce(sum(greatest(coalesce(sc.patient_portion_xaf, sc.amount_xaf) - coalesce(sc.amount_paid_xaf,0),0)),0)
  ...
  if round(p_total_amount_xaf,2) <> round(v_selected_outstanding,2) then
    raise exception 'Selected laboratory payment must be exactly % FCFA; received amount was % FCFA', v_selected_outstanding, p_total_amount_xaf;
  end if;
  ...
  -- Payment and lab financial state must agree before this transaction commits.
  if exists (
    select 1
    from lab_order_items loi
    join service_charges sc on sc.id = loi.service_charge_id
    where loi.id = any(p_lab_order_item_ids)
      and (loi.billing_status <> 'paid' or greatest(coalesce(sc.patient_portion_xaf, sc.amount_xaf) - coalesce(sc.amount_paid_xaf,0),0) > 0)
  ) then
  ...
  values (
    v_invoice_clinic_id, p_received_by, 'laboratory.selected_payment_collected', 'invoice', p_invoice_id,
    jsonb_build_object('visit_id',v_invoice_visit_id,'patient_id',v_invoice_patient_id,'lab_order_item_ids',p_lab_order_item_ids,'amount_xaf',p_total_amount_xaf,'method',p_method,'reference',nullif(trim(p_reference),''),'payment_id',v_payment_id)
  );
