-- ============================================================================
-- CRITICAL FIX: PHARMACY DISPENSING NOW GENERATES A PAYABLE INVOICE
--
-- Every other charge-generating event in this system (consultation
-- check-in, lab ordering) wraps its charge in an invoice immediately —
-- dispensing never did. This meant a dispensed prescription created a
-- real charge with genuinely no way to ever collect payment against it.
-- Fixed by calling open_invoice_for_charge right after the charge is
-- created, same as everywhere else.
-- ============================================================================
