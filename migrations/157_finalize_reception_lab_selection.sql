-- ============================================================================
-- MIGRATION 157: FINALIZE RECEPTION LAB SELECTION
--
-- When reception selects only some investigations from an ordered lab batch,
-- every other still-pending item in those same lab orders is moved to
-- deferred. Its unpaid charge is cancelled. The clinical order remains in
-- the record and can later be activated exactly once.
-- ============================================================================
...
begin
  ...
  select lo.clinic_id, lo.visit_id, lo.id into v_first_clinic_id, v_first_visit_id, v_order_id
  from lab_order_items loi join lab_orders lo on lo.id = loi.lab_order_id
  where loi.id = p_lab_order_item_ids[1]
    and loi.status = 'pending'
    and loi.billing_status in ('pending_payment','deferred')
  for update;

  if v_first_clinic_id is null then raise exception 'Selected laboratory investigation is no longer payable'; end if;

  select coalesce(active_role, role) into v_role from staff
  where id = p_staff_id and clinic_id = v_first_clinic_id and is_active;
  ...
  foreach v_item_id in array p_lab_order_item_ids loop
    select lo.clinic_id, lo.visit_id, loi.service_charge_id, loi.status::text, loi.billing_status
      into v_item_clinic_id, v_item_visit_id, v_charge_id, v_status, v_billing_status
    from lab_order_items loi join lab_orders lo on lo.id = loi.lab_order_id
    where loi.id = v_item_id for update;

    if v_item_clinic_id is null or v_item_clinic_id <> v_first_clinic_id or v_item_visit_id <> v_first_visit_id then
    ...
    if v_charge_id is null then raise exception 'Laboratory investigation has no financial charge'; end if;
    if not (v_charge_id = any(v_charge_ids)) then v_charge_ids := array_append(v_charge_ids, v_charge_id); end if;
  end loop;

  -- Any unselected pending item in the same lab order(s) becomes deferred.
  -- This removes its unpaid financial liability while preserving the clinical
  -- order for later activation. Already paid/authorized/performed items are
  -- never touched.
  for v_unselected in
    select loi.id, loi.service_charge_id, lo.clinic_id
  ...
  loop
    if v_unselected.service_charge_id is not null then
      update service_charges set status = 'cancelled'
      where id = v_unselected.service_charge_id and amount_paid_xaf = 0;
      if not found then raise exception 'An unselected laboratory charge has already received payment'; end if;
    end if;
    update lab_order_items set billing_status = 'deferred' where id = v_unselected.id;
    insert into audit_log (clinic_id, staff_id, action, entity_type, entity_id, details)
    values (v_unselected.clinic_id, p_staff_id, 'laboratory.item_auto_deferred', 'lab_order_item', v_unselected.id,
            jsonb_build_object('reason','Reception selected other investigations for payment','selected_item_ids',p_lab_order_item_ids));
  end loop;

  foreach v_charge_id in array v_charge_ids loop
    select * into v_charge from service_charges where id = v_charge_id for update;
    if v_charge.id is null then raise exception 'Laboratory charge not found'; end if;
  ...
  insert into audit_log (clinic_id, staff_id, action, entity_type, entity_id, details)
  values (v_first_clinic_id, p_staff_id, 'laboratory.selected_payment_prepared', 'visit', v_first_visit_id,
